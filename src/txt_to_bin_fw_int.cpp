...
int main(int argc, char** argv){
	...
	while(offset<edge_count){
		...
		curr = next;
		degree[index]++;
//		degree[index_a]++;////should only index ++
...
//	for(int i=0; i<64; i++){
    ...
    int sub_fw_beg_fd = open( "sub_fw_begin.bin",O_CREAT|O_RDWR,00666 );
	...
	int sub_fw_adj_fd = open( "sub_fw_adjacent.bin",O_CREAT|O_RDWR,00666 );
